...
export default function App() {
  ...
  async function openSettings() {
    // URL 해시에서 GOSSOcookie 자동 주입 (#gwcookie=xxx)
    let hashGwCookie = '';
    if (typeof window !== 'undefined' && window.location.hash) {
      const m = window.location.hash.match(/gwcookie=([^&]+)/);
      if (m) {
        hashGwCookie = decodeURIComponent(m[1]);
    ...
    setSettingsData({
      ...
      gwSession: hashGwCookie || gwSessionRef.current || '',
    });
    setSettingsMsg(hashGwCookie ? { text: 'GOSSOcookie가 자동으로 입력됐습니다. 저장 버튼을 눌러주세요.', type: 'ok' } : { text: '', type: '' });
  ...
  async function loadInstallDoc(item) {
    ...
    try {
      ...
      setInstallPanel({ name: data.name || item.label, content, attachments });
  ...
  async function submitCuReg() {
    ...
    try {
      const cuUserId = cuMyUserRef.current?.id;
      const descWithSig = cuRegForm.description + (cuUserId ? `\n\n@[${cuUserId}]` : '');
  ...
  return (
    <>
      ...
      {gwMailSending && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 3000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px' }}>
          <div style={{ width: '48px', height: '48px', border: '5px solid rgba(255,255,255,0.3)', borderTop: '5px solid #fff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
          <span style={{ color: '#fff', fontSize: '15px', fontWeight: 600 }}>그룹웨어 메일 전송 중...</span>
        </div>
      )}
      {licEmailModal && (
        ...
        <div className="sidebar">
          <div className="sidebar-header">
            <div className="sidebar-top">
              <span className="sidebar-title">Clickpad_v368</span>
          ...
          {(currentTab === 'clickup' || currentTab === 'license') && !hasClickupToken && (
            <div className="notes-list">
              ...
              {cuDetail.loading
                ...
                : <>
                  ...
                  <div className="task-detail-info">
                    <div className="task-detail-row"><span className="task-detail-label">태스크 ID</span><span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><span>{cuDetail.task.id}</span><button onClick={() => navigator.clipboard.writeText(cuDetail.task.id).then(() => showToast('복사됨'))} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '4px', padding: '1px 6px', fontSize: '11px', cursor: 'pointer', color: 'var(--text-muted)' }}>복사</button></span></div>
                    <div className="task-detail-row"><span className="task-detail-label">분류</span><span>{cuDetail.task.list?.name}</span></div>
                    <div className="task-detail-row"><span className="task-detail-label">작성자</span><span>{cuDetail.task.creator?.username || '-'}</span></div>

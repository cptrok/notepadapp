export default async function handler(req, res) {
  ...
  const prompt = mode === 'date'
    ? `다음은 "${channelName}" 채널의 ${date} 대화 내용입니다.\n아래 내용을 다음 형식으로 한국어로 정리해주세요. ${style}\n\n## 타이틀\n${titleInstruction}\n\n## 이슈사항\n- 해당 날짜에 발생한 핵심 이슈/문제/오류만 간결하게 항목별로 정리. 진행내역에서 다룰 세부 확인 내용이나 원인 분석은 제외하고 "무엇이 문제인가"만 작성\n\n## 진행내역\n- 대화의 흐름 순서대로 항목별로 정리. 무엇을 확인했는지, 어떤 결과가 나왔는지, 어떤 결정이 내려졌는지, 후속 조치는 무엇인지 구체적으로 작성. 담당자/작성자 이름은 포함하지 말 것. 내용을 빠짐없이 상세히 작성. 이슈사항과 중복되는 내용은 제외\n\n(해당 항목이 없으면 "없음"으로 표기)\n\n대화 내용:\n${text}`
    : `다음은 "${channelName}" 채널의 최근 대화 내용입니다.\n아래 내용을 다음 형식으로 한국어로 정리해주세요. ${style}\n\n## 타이틀\n${titleInstruction}\n\n## 이슈사항\n- 대화에서 발생한 핵심 이슈/문제/오류만 간결하게 항목별로 정리. 진행내역에서 다룰 세부 확인 내용이나 원인 분석은 제외하고 "무엇이 문제인가"만 작성\n\n## 진행내역\n- 대화의 흐름 순서대로 항목별로 정리. 무엇을 확인했는지, 어떤 결과가 나왔는지, 어떤 결정이 내려졌는지, 후속 조치는 무엇인지 구체적으로 작성. 담당자/작성자 이름은 포함하지 말 것. 내용을 빠짐없이 상세히 작성. 이슈사항과 중복되는 내용은 제외\n\n(해당 항목이 없으면 "없음"으로 표기)\n\n대화 내용:\n${text}`;
